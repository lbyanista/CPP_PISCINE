...
bool Fixed::operator>(Fixed const &fixed) const{
    // cout << "assignment operator > called" << ln;
    return (this->getRawBits() > fixed.getRawBits());
}

bool Fixed::operator<(Fixed const &fixed) const{
    // cout << "assignment operator < called" << ln;
    return (this->_fpoint < fixed._fpoint);
}
...
Fixed const &Fixed::min( const Fixed &lfixed, const Fixed &rfixed)
{
	if (lfixed < rfixed)
		return (lfixed);
	return (rfixed);
...
Fixed	const &Fixed::max( const Fixed &lfixed, const Fixed &rfixed)
{
	if (lfixed > rfixed)
		return lfixed;
    return rfixed;
